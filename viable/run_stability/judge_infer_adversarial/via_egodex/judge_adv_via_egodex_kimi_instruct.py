...
def run(infer, trial: str):
    ...
    """
    jsonl_file = os.path.join(DATA_ROOT, "adv_via_egodex.jsonl")
    if not os.path.exists(jsonl_file):
        print(f"[Skip] Data file not found: {jsonl_file}")
        return

    out_dir = os.path.join(RUN_EFF_DIR, "outputs_adversarial", "via_egodex", f"{MODEL_TAG}_trial{trial}")
    os.makedirs(out_dir, exist_ok=True)

    with open(jsonl_file, encoding="utf-8") as fh:
        lines = [line for line in fh if line.strip()]

    for idx, line in enumerate(tqdm(lines, desc=f"via_egodex/{MODEL_TAG}/trial{trial}")):
