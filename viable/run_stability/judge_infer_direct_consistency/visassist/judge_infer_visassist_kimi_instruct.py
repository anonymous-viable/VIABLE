...
MODEL_TAG  = "kimi_vl_a3b_instruct"
...
def run(run_id: int = 0):
    ...
    jsonl_file = os.path.join(DATA_ROOT, f"single_injected_visassist.jsonl")
    out_dir    = os.path.join(RUN_EFF_DIR, "outputs_stable", f"output_stable_run{run_id}_single_injected_visassist_{MODEL_TAG}")
    ...
    for line in tqdm(filtered_lines, desc=f"visassist/single_injected"):
