...
def run(run_id: int = 0):
    ...
    jsonl_file = os.path.join(DATA_ROOT, f"single_injected_walkvlm.jsonl")
    out_dir    = os.path.join(RUN_EFF_DIR, "outputs_stable", f"output_stable_run{run_id}_single_injected_walkvlm_{MODEL_TAG}")
    ...
    for line in tqdm(filtered_lines, desc=f"walkvlm/single_injected"):
