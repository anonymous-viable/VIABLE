...
# ── config ────────────────────────────────────────────────────────────────────
MODEL_LOCAL_PATH = "PATH/TO/Kimi-VL-A3B-Instruct"
...
def run(injection_type: str):
    ...
    jsonl_file = os.path.join(DATA_ROOT, f"{injection_type}_via_egodex.jsonl")
    out_dir    = os.path.join(RUN_EFF_DIR, "outputs_direct", f"output_direct_{injection_type}_via_egodex_{MODEL_TAG}")
    os.makedirs(out_dir, exist_ok=True)

    with open(jsonl_file, encoding="utf-8") as fh:
        lines = [line for line in fh if line.strip()]

    for line in tqdm(lines, desc=f"via_egodex/{injection_type}"):
