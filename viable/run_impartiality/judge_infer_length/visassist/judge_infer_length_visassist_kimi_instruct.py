...
FRAME_ROOT = "PATH/TO/full_samples_resized/VisAssist/frames"
...
def run(trial: str):
    ...
    jsonl_file = os.path.join(DATA_ROOT, "length_bias_I1_visassist.jsonl")
    out_dir = os.path.join(RUN_IMP_DIR, "outputs_length", "visassist", f"{MODEL_TAG}trial{trial}")
    os.makedirs(out_dir, exist_ok=True)

    with open(jsonl_file, encoding="utf-8") as fh:
        lines = [line for line in fh if line.strip()]

    for line in tqdm(lines, desc=f"visassist/length/trial{trial}"):
