...
sys.path.insert(0, os.path.join(RUN_IMP_DIR, "judge_infer_position_single"))
...
FRAME_ROOT = "PATH/TO/full_samples_resized/EgoDex/frames"
DATA_ROOT  = os.path.join(RUN_IMP_DIR, "data_length_bias")
...
def run(trial: str):
    ...
    jsonl_file = os.path.join(DATA_ROOT, "length_bias_I1_via_egodex.jsonl")
    out_dir = os.path.join(RUN_IMP_DIR, "outputs_length", "via_egodex", f"{MODEL_TAG}trial{trial}")
    os.makedirs(out_dir, exist_ok=True)

    with open(jsonl_file, encoding="utf-8") as fh:
        lines = [line for line in fh if line.strip()]

    for line in tqdm(lines, desc=f"via_egodex/length/trial{trial}"):
        sample = json.loads(line)

        sample_id = sample["sample_id"]

        # detect single vs dual record
        if "failure" in sample:
            label = sample["failure"]
            injection_type = "single"
            gt_labels = [label]
        elif "failure_pair" in sample:
            gt_labels = sorted(sample["failure_pair"])
            label = "_".join(gt_labels)
            injection_type = "dual"
        else:
            print(f"[Skip] {sample_id}: missing failure/failure_pair key")
            continue
