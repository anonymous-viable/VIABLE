...
from utils_effectiveness.inference_kimi import InferenceKimi
from utils_effectiveness.process_dataset import get_img_files

MODEL_LOCAL_PATH = "PATH/TO/models/moonshotai/Kimi-VL-A3B-Instruct"
MODEL_TAG = "kimi_instruct"
...
def run(infer, trial: str, judge_model: str):
    jsonl_file = os.path.join(DATA_ROOT, "via_egodex", f"via_egodex_{judge_model}.jsonl")
    if not os.path.exists(jsonl_file):
        print(f"[Skip] Data file not found: {jsonl_file}")
        return
    out_dir = os.path.join(RUN_IMP_DIR, "outputs_self_preference", "via_egodex", judge_model, f"{MODEL_TAG}trial{trial}")
    os.makedirs(out_dir, exist_ok=True)
    with open(jsonl_file, encoding="utf-8") as fh:
        lines = [line for line in fh if line.strip()]
    for line in tqdm(lines, desc=f"via_egodex/{judge_model}/trial{trial}"):
        ...
        if trial == "A":
            ...
            expected = "B"
        prompt = JUDGE_PROMPT.format(question=sample["question"], response_a=response_a, response_b=response_b)
        try:
        ...
        if predicted not in ["A", "B", "Tie"]:
            for choice in ["A", "B", "Tie"]:
                if choice in predicted:
                    ...
                    break
        record = {
            "sample_id": sample_id, "task": "via_egodex", "judge_model": judge_model, "trial": trial,
            "frame_path": sample.get("frame_path", ""), "question": sample["question"],
            "gt_answer": sample["gt_answer"], "self_prediction": sample["self_prediction"],
            "response_a": response_a, "response_b": response_b, "expected": expected,
            "predicted": predicted, "judge_raw_output": raw_output,
        }
        with open(out_file, "w", encoding="utf-8") as f:
            json.dump(record, f, ensure_ascii=False, indent=2)

if __name__ == "__main__":
    infer = InferenceKimi(MODEL_LOCAL_PATH)
    for trial in ["A", "B"]:
