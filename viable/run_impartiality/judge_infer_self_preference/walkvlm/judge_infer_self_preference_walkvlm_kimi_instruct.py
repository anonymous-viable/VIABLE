...
def run(infer, trial: str, judge_model: str):
    jsonl_file = os.path.join(DATA_ROOT, "walkvlm", f"walkvlm_{judge_model}.jsonl")
    if not os.path.exists(jsonl_file):
        print(f"[Skip] Data file not found: {jsonl_file}")
        return
    out_dir = os.path.join(RUN_IMP_DIR, "outputs_self_preference", "walkvlm", judge_model, f"{MODEL_TAG}trial{trial}")
    os.makedirs(out_dir, exist_ok=True)
    with open(jsonl_file, encoding="utf-8") as fh:
        lines = [line for line in fh if line.strip()]
    for line in tqdm(lines, desc=f"walkvlm/{judge_model}/trial{trial}"):
        ...
        if predicted not in ["A", "B", "Tie"]:
            for choice in ["A", "B", "Tie"]:
                if choice in predicted:
                    ...
                    break
        record = {
            "sample_id": sample_id, "task": "walkvlm", "judge_model": judge_model, "trial": trial,
